...
async function save(reqVeterinaryAssociations) {
  const existentVeterinaryAssociations = await VeterinaryAssociation.findAll({
    where: { [Op.or]: reqVeterinaryAssociations },
  });
  existentVeterinaryAssociations.forEach((eva) => {
    reqVeterinaryAssociations = reqVeterinaryAssociations.filter(
      (rva) => rva.veterinaryId != eva.veterinaryId && eva.vetId != eva.vetId
    );
  });
  reqVeterinaryAssociations.forEach(async function eachVeteriaryAssociation(
    veteriaryAssociation
  ) {
    await VeterinaryAssociation.create(veteriaryAssociation);
  });
  return await VeterinaryAssociation.findAll({
    where: { [Op.or]: reqVeterinaryAssociations },
  });
}
...
async function findTemporalAssociationByCode(associationCode) {
  temporalVeterinaryAssociation = temporalVeterinaryAssociations.find(
    (temporalVeterinaryAssociation) =>
      temporalVeterinaryAssociation.code == associationCode
  );
...
async function returnCompleteTemporalAssociation(
  temporalVeterinaryAssociation
) {
  const veterinary = await veterinaryService.findOneByMP(
    temporalVeterinaryAssociation.mp
  );
...
async function findAllDataByRegentId(regentId) {
  ...
  var vetDataList = await vetService.findAllVetDataByIds(
    vets.map((vet) => vet.id)
  );
  vets.forEach((vet) => {
    vetAsDataListByRegent.push({
      vetData: vetDataList.find((vdl) => vdl.vet.id === vet.id),
      veterinaryData: vetDataList.find((vdl) => vdl.vet.id === vet.id)
        .regentData,
    });
  });
...
async function findAllDataByVeterinaryId(veterinaryId) {
  ...
  var vetAssociationList = await VeterinaryAssociation.findAll({
    where: { veterinaryId: veterinaryId },
  });
  var vetDataList = await vetService.findAllVetDataByIds(
    vetAssociationList.map((val) => val.vetId)
  );
  var veterinaryDataList = await veterinaryService.findAllVeterinaryDataByIds(
    vetAssociationList.map((val) => val.veterinaryId)
  );
  vetAssociationList.forEach((val) => {
    vetAsDataListByVeterinary.push({
      ...
      veterinaryData: veterinaryDataList.find(
        (vydl) => vydl.veterinary.id === val.veterinaryId
      ),
    });
  });
...
async function findAllVeterinariesByRegentId(regentId) {
  ...
  var vetAssociationList = await VeterinaryAssociation.findAll({
    where: { vetId: vetIds },
  }); //busco todas las asociaciones que tienen las vets donde soy reg
  var veterinaryDataList = await veterinaryService.findAllVeterinaryDataByIds(
    vetAssociationList.map((val) => val.veterinaryId)
  ); //toda la data de los co veterinarios
  vetAssociationList.forEach((val) => {
    veterinaryDataListByRegent.push({
      id: val.id,
      vetData: vetDataList.find((vdl) => vdl.vet.id === val.vetId),
      coveterinaryData: veterinaryDataList.find(
        (vydl) => vydl.veterinary.id === val.veterinaryId
      ),
    });
  });
